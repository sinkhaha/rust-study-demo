...
Rust的所有权机制规定了一个值只能有一个所有者。当出现不符合所有权机制的情况时要怎么处理，如以下场景
...
1. a、b、c三个智能指针的引用计数都是一样的，共同指向堆上相同的数据，即共享堆上的数据，说明堆上的数据有3个所有者
...
2. Arc 内部的引用计数使用了 Atomic Usize ，而非普通的 usize
   ...
   > Atomic Usize 是 usize 的原子类型，它使用了 CPU 的特殊指令，来保证多线程下的安全。因为在于原子化或者其它锁虽然可以带来的线程安全，但是都会伴随着性能损耗，所以Arc的性能不如Rc
...
2. 是一个只读的引用计数器，不能获取到指向数据的可变引用，需要配合其他数据结构一起使用
...
因为 Rc 是一个只读的引用计数器，所以无法拿到 Rc 结构内部数据的可变引用，即无法通过它来修改数据。可以配合其他数据结构一起使用，如内部可变性的 `RefCell` 类型 或 互斥锁 `Mutex` 或 读写锁RwLock（Rc配合RefCell，Arc配合Mutex/Rwlock）。



1. 外部可变性：用 let mut 显式地声明一个可变的值，或者，用 &mut 声明一个可变引用时，编译器可以在编译时进行严格地检查，保证只有可变的值或者可变的引用，才能修改值内部的数据

2. 内部可变性：有时希望能够绕开编译时的检查，对并未声明成 mut 的值或者引用，也想进行修改。也就是，在编译器时，值是只读的，但是在运行时，这个值可以得到可变借用，从而修改内部的数据
...
RefCell绕过了 Rust 编译器的静态检查，允许我们在运行时，获取对某个只读数据的可变借用；RefCell也是线程不安全的。
...
fn main() {
   ...
   // 如果去掉大括号：可以编译通过，但是运行时会报错'already mutably borrowed: BorrowError'。可以看到，所有权的借用规则在此依旧有效，只不过它在`运行时检测`。
    {
        // 通过使用 RefCell 的 borrow_mut() 方法，来获得一个可变的内部引用，然后通过解引用对它做加 1 的操作
...
数据结构的 downstream 需要 Rc 内部嵌套一个 RefCell，来获得数据的可变借用，即 `Rc<RefCell<T>>`，这样DAG就可以正常修改
...
Rust提供一种机制：Box::leak()，可以创建不受栈内存控制的堆内存，从而绕过编译时的所有权规则。
